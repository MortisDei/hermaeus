...
| E2 | `/tmp/hermaeus-r33-tests-release-current/r33-final-release-current.trx` | Release sequential suite: `2,695` passed, `17` skipped, `0` failed, `2,712` total. |
| E3 | Direct final coverage command using `scripts/coverage.sh`'s inner `dotnet test` command; `/tmp/hermaeus-r33-coverage-current/*/coverage.cobertura.xml` | `49,853/76,443` lines, `65.21%` line coverage, above the `60%` ratchet. The wrapper's first restricted-host invocation exited silently; the exact command passed on the approved host. |
| E4 | `dotnet run --project src/Tools/R33Driver/R33Driver.csproj --no-build -- --settings-path /tmp/hermaeus-r33-driver-final-current/settings/settings.json --data-root /tmp/hermaeus-r33-driver-final-current/data --workspace /tmp/hermaeus-r33-driver-final-current/workspace` | Production composition with explicit scratch substitutions returned `ok:true`, an `Applied` changed/readback-verified Agent receipt, cancelled benchmark evidence, one RAG generation/query, Chat retrieval context, voice completion, Lab failure cleanup, Lab Apply/reopen, and shutdown. It is not native-runtime or GUI proof. |
...
| B8 Agent/RAG/Lab UX and Doctor targets | **owner validation required** | Existing `AgentView.axaml`, `RagView.axaml`, and `LabView.axaml` retain the job-oriented hierarchy; `MainWindowViewModel`, `DoctorService`, `DoctorViewModel`, `ServicesViewModel`, `ServicesView.axaml`, and `ServicesView.axaml.cs` now carry typed target identity, unavailable feedback, selection, scroll, and focus mapping. `DoctorAdvisoryTests.Server_specific_doctor_findings_keep_a_typed_remediation_target`, `LabViewModelTests`, `RagViewModelWatchedSourceTests`, `RagQuestionBoxTests`, and the structural guards cover the local projection. `75409f1` and `cc494ac`. | Static XAML and a view-model test cannot prove keyboard focus, DPI, resize, accessibility, contrast, usable artifact width, or the owner walkthrough of Agent/RAG/Lab. Those remain owner validation. |
...
| B9 integrated verification, docs, package, and owner platform evidence | **owner validation required** | `CHANGELOG.md`, `docs/features.md`, `docs/user-guide.md`, `docs/agent.md`, `docs/security-review.md`, `docs/review/r33/README.md`, the v0.41.0-beta package, E1-E5, and E3 provide local integrated evidence. `cc494ac` records the verified local closeout. | Owner still controls Linux visible launch, Windows build/runtime/GUI validation, actual R33 PR merge checks, CodeQL enforcement, and release/tag/publication. No publication action was taken. |
...
| V14 optional editor | **conditionally rejected/not earned with evidence** | `docs/review/r33/04-workspace-editor-decision.md:102-128` requires a real Monaco versus AvaloniaEdit cross-platform/offline/security/resource spike. The repository retains AvaloniaEdit, adds no Monaco/WebView dependency, and has no Windows/Linux renderer, worker, or bridge proof. This is an intentional gate failure, not a missing mandatory repair. |
...
| Agent/RAG/Lab UX | **owner validation required** | Existing `AgentView.axaml`, `RagView.axaml`, `LabView.axaml`, `ServicesView.axaml`, relevant ViewModels, layout guards, and E4 preserve the hierarchy and add target/status projections. Rendered density, focus, DPI, resize, and owner walkthrough remain. |
...
| Monaco decision | **conditionally rejected/not earned with evidence** | `docs/review/r33/04-workspace-editor-decision.md:102-128` requires a measured offline/native/cross-platform spike. No gate was earned and no WebView/Monaco dependency was added; AvaloniaEdit remains the fallback. |
...
| --- | --- | --- |
| C1 Monaco/WebView editor spike | **conditionally rejected/not earned with evidence** | The required comparison protocol and ship gate are `docs/review/r33/04-workspace-editor-decision.md:102-128`. No offline worker/native backend/bridge/resource measurements exist for both owner platforms, so the correct R33 outcome is retained AvaloniaEdit, not a fake spike pass. |
...
`15104d1`, and two policy revalidation cases in `1601413`. The final E1/E2
result is therefore `2,695` passed, `17` skipped, `2,712` total, or **twenty
more passing cases than the planning baseline**, not eight. `cc494ac` added no
new test case; it extended existing fakes/assertions and production paths.
