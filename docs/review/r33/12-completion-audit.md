...
- **owner pass (scoped)** means a real owner observation passed the named
  workflow on the named platform. It does not widen to other platforms,
  controls, models, or lifecycle paths.
- **unresolved** means the evidence narrows the fault boundary but does not
  prove the remaining mechanism or acceptance claim.
...
| E15 | 2026-09-13 pre-dogfood correction: reusable verification scratch helper, bash smoke coverage for success/failure/SIGINT/stale/unrelated paths, fresh Release driver, and isolated packaged Linux signal checks | `scripts/verification-scratch.sh` passed syntax and cleanup smoke checks, and the Release driver returned `ok:true` with its isolated Agent, benchmark, RAG, Chat, voice, Lab, Apply/reopen, and shutdown receipt. The direct packaged SIGINT check left the app alive with `CleanExit:false` and `LastOperation:"running"`; the exact test process was then stopped with SIGTERM and exited 143, also without a clean marker. This classifies the current `CleanExit:false` as console-interrupt/harness termination evidence that bypasses the Avalonia close/tray path, not as a demonstrated product shutdown defect. At this checkpoint the historical exit-139 process was still unclassified; E17 and E18 supersede that classification. |
| E16 | 2026-09-13 pre-dogfood authority and packaging correction: persisted Agent prepared-mutation authority, exact approval binding, direct draft/apply refusal, bounded native startup evidence fallback, reusable scratch driver, and fresh package | Debug and Release solution builds completed with `0` warnings and `0` errors. The focused authority, empirical-index, and runtime-evidence set passed `128/128`; each complete sequential suite passed `2,737`, skipped `17`, failed `0`, total `2,754`. The fresh Release driver returned `ok:true` with `outcome:"Applied"`, `verified:true`, and `changed:true`, and no namespace scratch root remained. The fresh archive is `dist/hermaeus-0.41.0-beta-linux-x64.tar.gz`; `sha256sum -c` passed for SHA256 `2900d63d389f8088188ca9bf87f72677d99c271993c49e64c1315e31acfc1ee7`, with ELF/executable/layout/no-PDB checks passing. Against that exact package, isolated run `hermaeus-package-shutdown-final.rHXRLK` recorded `CleanExit:false`, `LastOperation:"running"`; SIGINT left the exact app PID alive, and bounded SIGTERM produced exit `143` without a clean marker. This is console-interrupt/harness evidence that bypasses the Avalonia close/tray coordinator, not a demonstrated product shutdown defect. E17 and E18 later attribute the historical exit-139 process and reproduce its invalid draft-triggered failure class; normal packaged window close and all existing GUI interaction cells remain `NEEDS OWNER VALIDATION`. |
| E17 | Retained owner dogfood toast plus the system journal window on 2026-09-12, local AEST, for the AutoTune target `NVIDIA-Nemotron-3-Nano-4B-UD-Q4_K_XL.gguf` | The old `b10924` managed executable was recorded in the persisted settings. The candidate-0 attempt started at `21:03:20` with Hermaeus log PID `803762`, port `41387`, `--ctx-size 131072`, four threads, CPU placement (`--n-gpu-layers 0`), and the inherited `mtp-gemma-4-E2B-it-Q4_0.gguf` draft. It logged a native exit `139` at `21:03:30`; the kernel recorded a `libllama.so.0` segfault at `21:03:28`. Five sibling candidates also segfaulted in the same draft-load phase. The Hermaeus OS parent PID and the complete persisted argv were not retained. This establishes child attribution and the old cross-model companion contamination, not the exact native fault mechanism. |
| E18 | Bounded approved-host replay on 2026-09-14 using installed `b10930`, plus a matching no-draft control, with no Hermaeus settings or owner-data writes | The CPU-only invalid-pair replay used the reconstructed target/draft shape, `131072` context, four threads, and `--n-gpu-layers 0`; it loaded Nemotron, loaded the Gemma MTP draft, and exited `139`. Kernel PID `47756` faulted in `libllama.so.0` at the same `+0x22d792` library offset; no matching `coredumpctl` record was available. The no-draft Nemotron control with the same CPU placement loaded, listened on loopback, and exited only during bounded cleanup. This reproduces the failure class and ties it to speculative draft initialization, but it is not an exact `b10924` binary replay and does not prove the upstream mechanism is fixed. |

## Exit-139 forensic closeout

The historical event is now attributed to the managed `llama-server` child,
not Hermaeus UI code or a WebView host. The operation was a Models-card
AutoTune attempt for the Nemotron target while the Gemma source runtime was
the active managed configuration. The retained settings identify the old
runtime as `b10924`, the backend intent as Vulkan, the configured context as
`131072`, and the source configuration as four generation threads. Candidate
0 changed placement to CPU, which explains the reconstructed `--n-gpu-layers 0`
below. The historical target and inherited Gemma MTP draft are identified by
the toast and child log; the target's effective device allocation was not
persisted.

For candidate PID `803762`, the reconstructable launch shape is:

```text
b10924/llama-server
-m NVIDIA-Nemotron-3-Nano-4B-UD-Q4_K_XL.gguf
--port 41387 --host 127.0.0.1 --ctx-size 131072 --threads 4
--fit off --n-gpu-layers 0 --parallel 1 --cache-reuse 256
--spec-type draft-mtp --spec-draft-model mtp-gemma-4-E2B-it-Q4_0.gguf
```

This is a source-and-settings reconstruction, not a persisted full argv.
Exact absolute model paths, any prompt-thread setting, projector flag,
draft-layer setting, extra arguments, and other capability-dependent flags
were not retained. Hermaeus PID `800370` logged and owned the child launches,
but the kernel journal did not preserve the OS parent PID relationship.

The causal classification is deliberately split:

| Layer | Final classification |
| --- | --- |
| Process and operation | **VERIFIED EXISTING:** `llama-server`, during the Nemotron AutoTune candidate, with six sibling attempts showing the same failure phase. |
| Hermaeus trigger | **VERIFIED EXISTING:** the pre-repair `BuildModelTuneProbe` cloned the active ChatServer configuration, changed the target model, and retained the source draft/projector/extra-argument state. This supplied an unverified cross-model draft to the target. |
| Native fault trigger | **REPRODUCED:** the installed `b10930` CPU-only Nemotron plus Gemma MTP pair loaded the target and draft, then exited `139`; the no-draft control loaded and listened normally. |
| Native fault mechanism | **UNRESOLVED:** the kernel records `libllama.so.0 +0x22d792`, but no symbolic native stack or usable coredump was retained. The result may be an upstream guard/bug triggered by the invalid pair; the exact upstream fix is not established. |
| WebView/Monaco | **CLOSED FOR THIS EVENT:** the retained operation and native fault evidence are the llama child and speculative initialization. No WebView lifecycle or editor operation is present in the captured artifacts, so no Monaco A/B was warranted. Monaco remains subject to its independent platform gate. |
| Current Hermaeus path | **NEEDS OWNER VALIDATION:** the target probe now starts from a fresh config and only verified target companions; E8 proved the target-only command and source restoration, but not a visible card click or every backend/placement combination. |

## Shutdown evidence reconciliation

The owner-live Linux tray Quit path is a scoped **OWNER PASS**. The packaged
application exited with code `0`, both managed llama children were gone, and
the lifecycle journal recorded `CleanExit:true` with the shutdown owners
reporting `timedOut:false`.

The later direct package SIGINT check is a different boundary. SIGINT left the
application alive with `CleanExit:false` and `LastOperation:"running"`; bounded
SIGTERM then returned `143` without invoking the clean marker. `Program.Main`
does not register a console interrupt handler, so this is harness termination
evidence that bypasses the Avalonia window/tray coordinator. It does not
reopen the tray shutdown repair. Normal window-close, restart handoff, and
Windows lifecycle behavior remain **NEEDS OWNER VALIDATION**.

## Internal pet naming reconciliation

The implementation identifiers are now neutral: `PetPackage*`,
`IPetPackageCatalog`, `DesktopPet*`, and `PetSprite`. The public compatibility
name remains factual **ChatGPT Pet v2** in user-facing docs, manifest guidance,
provenance, and UI copy. The data-only boundary, local-only behavior,
disabled-by-default setting, Moss asset, sprite version `2`, persistence, and
security validation are unchanged. No settings key, package schema, or
...
the exact historical `b10924` binary is no longer installed. E18 reproduces
the same invalid draft-triggered exit-139 class on `b10930`, while a matching
no-draft control loads normally.
...
failure. The exit-139 process and trigger are classified in E17/E18, while
the native fault mechanism remains `UNRESOLVED`. Normal packaged window close
remains `NEEDS OWNER VALIDATION`. Windows, restart handoff, and broader
owner-live GUI checks remain open.
...
- visible Models-card activation and additional managed runtime/model/GPU
  AutoTune and model-switch recovery combinations. Retain E17/E18 if the
  historical `b10924` fault cannot be reproduced on Windows;
...
## Windows RTX 4060 owner-validation checklist

Use a fresh Windows 11 portable/package run on the RTX 4060 laptop. Record the
package identity and SHA256, commit identity, Windows build, GPU and driver,
selected backend, model/runtime identity, data-root choice, and every owned
process PID, start time, executable, and argv. Screenshots prove pixels only;
keep them separate from process, interaction, and clean-exit receipts.

| Stage | Required checks | Closeout evidence |
| --- | --- | --- |
| 1. Lifecycle, runtime, backend | Launch from a path containing spaces; confirm the real window and tray behavior; start, stop, and restart managed runtime and embedding/voice children; exercise normal window close and tray Quit; record CUDA/Vulkan/CPU as actually selected and effective; repeat after a failed startup. | Clean exit and child inventory, lifecycle journal, runtime log, exact launch receipts, and a visible launch/close observation. |
| 2. Lab, Benchmark, AutoTune | Run Models-card AutoTune with a loaded source and different unloaded target; confirm target-only draft/projector/extra-argument state, source suspension/restoration, cancellation, failure cleanup, and profile-save ordering. Run Lab discovery, one controlled candidate, cancel/restore, Apply/reopen, and benchmark cancellation during preparation and a case. | Per-run requested/resolved/launched/effective/telemetry identities, candidate outcomes, retained details, no leaked child, and visible status. Do not treat a requested backend or GPU layer count as effective proof. |
| 3. Agent, RAG, editor | Exercise parent/child approval, full-file rewrite, artifact inspection, New Task with delayed work, failure/cancel/conflict, RAG ingest/query/citations/traces/cancel, and Doctor target navigation. Check Monaco/WebView2 startup, workers, offline resources, focus, keyboard/IME, DPI, resize, large artifacts, and teardown. If WebView2 actually fails, capture Monaco versus AvaloniaEdit fallback A/B; do not disable Monaco by default from a Linux-only concern. | Filesystem/readback receipts, task/run ids, citation identity, editor screenshots plus control-flow notes, WebView2/worker logs, and complete shutdown process inventory. |
| 4. Pet, audio, and UX | Confirm ChatGPT Pet v2 remains disabled by default; enable Moss, import/select a package, drag in both directions, resize/DPI clamp, restart persistence, and overlay teardown. Exercise voice playback/audio feedback, settings save/error state, narrow/wide layout, light/dark contrast, keyboard focus, and Doctor/remediation return paths. | Visible screenshots and interaction notes, persisted settings diff limited to the intended fields, audio completion/failure outcome, and no package script/network/chat-content behavior. |

Every stage is a separate Windows result. A Linux owner pass, a native probe,
or a source test does not close the RTX 4060 Windows cell.
