...
| Benchmarks "Best Overall" column, ranked across all suites | r25 follow-up (owner request) | The owner asked for a first column naming the model that performed best across **all** suites, with per-suite scores on click, alongside renaming Rankings to Per-Suite Rankings. The rename shipped; the column did not. It needs per-suite aggregation that does not exist yet: `BenchmarkInsightsReport` carries an overall board (ranked on a shared case set) and per-**tag** leaderboards, but nothing keyed by suite. That means a new model type, the aggregation, a view model and a tab. Deferred rather than half-built. Most of the groundwork is already in place from r25 doc 04: the Insights tab's Best overall card ranks on a shared case set, names the axis that won, and opens to a per-case breakdown, so this is the same idea keyed by suite instead of case. **Strong r26 candidate.** |
