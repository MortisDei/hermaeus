...
| Nav-icon cursor flicker: mechanism still unconfirmed | r29 1.5 | Two fixes have shipped against theorised causes (tooltip placed below the control, nav row made one continuous hand-cursor region) and the flicker survives both. The owner reports it at the very outer edge of each button. The leading hypothesis is that the tooltip popup, which opens 6px below the button, takes pointer-over when the pointer sits at that edge, so the control counts as exited and the loop restarts; that would explain why two CURSOR fixes did not help, because the cursor region was never the problem. It is a hypothesis. r29 deliberately shipped no third speculative fix. **The experiment, which needs a human at the pointer:** remove `ToolTip.Tip` from one nav button, run the app, hover its outer edge. If the flicker stops, the tooltip is the mechanism and the fix is `<Setter Property="IsHitTestVisible" Value="False" />` on the `ToolTip` style in `AppStyles.axaml` (nothing in this app has a clickable tooltip), plus raising `ToolTip.VerticalOffset` to 10. If it still flickers with no tooltip, the mechanism is something else and the hypothesis above is wrong. |
