...
binaries nobody can regenerate. The composition there is not a scaled copy of
`MossIcon.axaml`: that control was drawn for a 16px inline glyph, and enlarged,
its ears read as horns and its spectacle band as a blindfold. The icon draws
the same character at icon scale, with round brass spectacles.

That generator output is a programmatic approximation, not commissioned art.
It is legible at 16px, which is the bar the Tree Ring mark failed, but a
designed replacement would be an improvement and can drop straight into the
same filenames.
