...
| Backups | Backup excludes the fallback vault and legacy key filename. The active fallback key lives outside the data root, so it is outside the backup scope as well. Restore preflights every file, normalizes both archive separators, ignores directory entries, rejects traversal, rooted, drive-qualified, UNC, path-prefix, case-variant sibling, and reparse-point escapes, enforces file-count and per-entry and total uncompressed-size budgets, and refuses existing-file overwrites unless explicitly allowed. Each archive is expanded beneath a data-root staging directory with actual-byte checks, then committed with rollback-safe moves after size and safety checks pass. | Credentials are not restored and must be re-entered on another machine. Backup archives can still contain chat, RAG, benchmark, Agent, and path metadata. The default restore limits are bounded but users should still restore only archives they intend to trust. |
...
| Local server binding | Managed `llama-server` always receives `--host 127.0.0.1` before user extra args. When the selected runtime proves `--cors-origins`, Hermaeus also passes `http://localhost,http://127.0.0.1`, preventing arbitrary browser origins from reading the loopback API by default. Health checks target `127.0.0.1`. Trust scans detect `--host value`, `--host=value`, listen-host forms, and `--listen`. | Pinned llama.cpp b10034 processes later duplicate `--host` flags in order, so a user-supplied network-facing host in Extra Args overrides loopback. Privacy Audit warns about this configuration, but it is not blocked. The CORS tightening does not authenticate the API and does not change external/custom server profiles. |
...
| Experience and empirical identities | `experience.db` is a local descriptive index whose records retain evidence origin, normalized outcome, raw-source links, and path-redacted export. Runtime/model/hardware/configuration v2 stable ids exclude executable and model paths, hostnames, user names, serial numbers, and raw extra arguments. Capability parameters are bounded parsed facts and never raw help text. Experience and capability observations do not feed Agent safety, approval, workspace, API authority, or destructive confirmation. | Local database rows can still describe project scope and model/runtime facts, and benchmark metadata retains existing user-selected model paths outside the portable v2 identity. Backups remain sensitive local artifacts. |
| Recommendation persistence | R32 recommendations are typed, deduplicated, and stored in normalized `experience.db` tables. Proposed patches are canonicalized, bounded, path-free, and reject secret-shaped fields; evidence, decisions, and bounded rollback pre-images are separate records. Managed-server Apply and Undo revalidate target identity, write only through `SettingsService`, reconcile pending records by observation, and never restart a process or route a model automatically. Model guidance is review-only. Recommendations do not grant Agent authority or approve their own application. | Evidence and rollback metadata can still describe local hardware, runtime, configuration, and user decisions. The active database and backups remain sensitive local artifacts. |
| Runtime telemetry and GPU Fit | Samples are bounded, process-instance scoped, and retain source/trust state. Process working set is read only for the matching live process. Whole-device GPU memory is labelled as a device total and cannot produce a model discrepancy. Persisted GPU Fit evidence requires an exact path-free v2 fingerprint match and never changes settings or analytical maths. | Process/resource counters reveal local hardware capacity and runtime resource use inside the local database and its backups. Per-process VRAM remains unavailable on platforms where no trustworthy counter is implemented. |
| Whole-workload resource admission | Managed server, Lab, local voice-process, ONNX load, and embedding backfill owners use the shared coordinator when composed in production. Plans retain Unknowns, use bounded expiring reservations, and preserve whole-device totals as unattributed. Admission has no eviction, process-kill, provider-switch, or settings-mutation authority. | Per-process VRAM and some ONNX or voice memory sizes remain Unknown. Reservations are in-memory coordination tokens, not physical memory reservations, so external pressure can still make a launch fail. |
| Adaptive managed launches | The saved envelope defaults to Fixed. Advise never starts a changed configuration. AdaptAtLaunch uses a bounded deterministic candidate list, refuses Unknown admission, acquires a fresh reservation for each attempt, retries only resource-exhaustion failures, awaits owned-process teardown, and requires structured effective context and placement evidence before accepting a changed launch. Candidate overlays remain transient, so a failed or successful compromise cannot overwrite saved settings. | The installed runtimes in the review environment do not expose effective GPU placement or fit changes through `/props`, so adaptive launch remains unavailable for those observed changes. The fallback classifier is deliberately conservative and may leave an ambiguous runtime failure as Unknown. |
| Live telemetry and audio feedback | Chat request metrics are direct provider evidence and remain available without a polling dashboard. The shared sampler is bounded and process-identity scoped; the Chat flyout obtains the exact managed child PID and start time from Services before sampling. Health policy does not alert on high GPU utilization alone, suppresses Unknown evidence, and deduplicates by runtime/model identity. Audio cues use one explicit event service, a bounded queue, visual equivalents, TTS suppression, validated temporary WAV data, and argument-only playback invocation; playback failure cannot fail the originating operation. | Process/resource counters reveal local hardware capacity and runtime resource use inside the local database and its backups. Per-process VRAM remains unavailable where no trustworthy counter exists. Audibility, packaged playback, and reduced-motion/assistive-setting behavior remain manual platform checks. |
| Lab isolated runtimes, recipes, and Apply | Lab clones a selected server into a dedicated `ServerProcessManager`, chooses a temporary port, forces loopback, rejects host/port overrides, and never saves settings while running. Ownership recovery matches PID, start time, and executable SHA256 before killing anything; a mismatch remains Unknown. A missing ownership manifest is known empty, but a present manifest that is malformed, truncated, unreadable, or otherwise indeterminate remains untouched and blocks recovery and ownership mutation. Bounded recipes send the user-controlled prompt only to that loopback runtime, persist its hash rather than its body, and store per-configuration evidence slices containing fingerprints, output hashes, counters, and path-redacted provenance. External draft/EAGLE plans require distinct non-link assets, verified model-manifest hashes, exact runtime capability, tokenizer/vocabulary compatibility, and EAGLE target binding; the persisted Lab configuration carries the companion stable id rather than its path. Shared-prefix experiments reconstruct bounded prompts in memory and persist only their hashes; no normal Chat or Agent content is sampled. Apply is a separate explicit review, revalidates server/runtime/model identity, requires the declared quality metric where applicable, clones settings, and uses `ISettingsService.SaveAsync`. | Temporary port reservation has the normal close-then-bind race and launch can fail honestly if another process wins it. A verified abandoned Lab child is terminated at next app startup. An unreadable ownership manifest intentionally leaves cleanup unresolved until the evidence becomes readable. Typed recipe content reaches the selected local model process. GGUF metadata is self-described, so the runtime can still reject or mishandle a pair after Hermaeus's preflight; launch failures remain evidence, never compatibility success. Internal experiment evidence and backups still reveal local model behavior and hardware capacity. |
...
| Recall index | `recall.db` is a local, unencrypted index (FTS5 keyword + embeddings) over content that already exists elsewhere in the app (conversations, agent tasks, memories, RAG chunks) - it is a second searchable copy of that content, the same way `ConversationStore`'s own FTS table already is, not a new category of exposure. On by default with a visible switch (Settings > Memory), a genuine "Clear index" delete, per-conversation exclusion, and honest size reporting. Indexing is a bounded background pass, never on the chat send path. Optional chat-side injection (off by default) surfaces retrieved Recall excerpts through the exact same citation-pill UI RAG/Memory injection already use. | Chat-side Recall injection, when enabled, sends the relevant excerpts to whichever chat provider is active for that turn, local or remote, the same as RAG/Memory injection already do - it does not add a new remote-exposure class, but it does add another path by which local content can reach a remote provider if one is configured. `recall.db` is covered by the same backup/restore posture as every other store (not separately encrypted). |
...
| Hugging Face artwork | The Models browser reads only the string-valued, 2,048-byte-capped `cardData.thumbnail` already returned by the selected model-card request. Relative and canonical repository-file values must resolve to the selected immutable tree revision. Other artwork is limited to HTTPS `huggingface.co` paths, with automatic redirects disabled, five-hop manual redirects, and an exact reviewed delivery-host set. No artwork request carries credentials. Response bodies are streamed and capped at 2 MiB; declared MIME, magic bytes, PNG/JPEG/WebP headers, animation flags, dimensions, decoded-pixel estimates, and post-Avalonia dimensions are checked before display. Cache writes are atomic, keyed by repo, exact revision, source identity, and content hash, bounded with LRU eviction, and cleared separately from models/manifests. | Artwork remains optional decoration and can expose the selected repository and standard network metadata to Hugging Face. A source-controlled delivery-host list needs review when Hugging Face changes its download architecture. Cache metadata and image bytes remain local user data until Clear; the cache is excluded from Data Root backups and must be reacquired after restore. |
...
fingerprints, remembered approvals, token scopes, or destructive confirmations.
MCP results without a structured `isError` status remain `Unknown`, even when
their response prose claims success.
...
- Restore refuses duplicate target paths and archives over the default 10,000
  file-entry, 128 MiB per-entry, or 512 MiB total uncompressed budgets before
  extraction. It also checks actual expanded bytes while copying into a
  data-root staging directory, then commits with rollback-safe moves.
...
- Every call is logged (verified token name, self-reported client hint,
  ...
  self-reported name.
- Agent execution routes are not mapped. Their v1 scope is additive and disabled
  for old and new tokens, contains no approval operation, binds saved workspace,
  model, Project, ownership, and concurrency policy, and remains inert until one
  service serializes all task mutations for Desktop and Local API.

Required follow-up: implement one Agent task-mutation owner and a Desktop scope
editor before mapping the conditional routes in `docs/agent-api.md`. See also
`docs/security-roadmap.md` ("Local API per-server tool-level scoping").
