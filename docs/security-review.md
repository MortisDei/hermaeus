...
| Lab isolated runtimes and Apply | Lab clones a selected server into a dedicated `ServerProcessManager`, chooses a temporary port, forces loopback, rejects host/port overrides, and never saves settings while running. Ownership recovery matches PID, start time, and executable SHA256 before killing anything; a mismatch remains Unknown. Exportable run evidence stores fingerprints, hashes, counters, and path-redacted provenance, not raw extra arguments, prompt/output bodies, or token values. Apply is a separate explicit review, revalidates server/runtime/model identity, clones settings, and uses `ISettingsService.SaveAsync`. | Temporary port reservation has the normal close-then-bind race and launch can fail honestly if another process wins it. A verified abandoned Lab child is terminated at next app startup. Internal experiment evidence and backups still reveal local model behavior and hardware capacity. |
