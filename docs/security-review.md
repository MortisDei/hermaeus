...
| Live telemetry and audio feedback | Chat request metrics are direct provider evidence and remain available without a polling dashboard. The shared sampler is bounded and process-identity scoped. Health policy does not alert on high GPU utilization alone, suppresses Unknown evidence, and deduplicates by runtime/model identity. Audio cues use one explicit event service, a bounded queue, visual equivalents, TTS suppression, temporary generated WAV data, and argument-only playback invocation; playback failure cannot fail the originating operation. | Ordinary Chat does not yet attach an exact managed process launch identity to the telemetry flyout, so process counters remain Unknown there until the live identity gate runs. Audibility, packaged playback, and reduced-motion/assistive-setting behavior remain manual platform checks. |
