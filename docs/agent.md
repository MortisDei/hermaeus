...
counts. The suite action and each row's Run action are disabled while
definitions are loading or until a model and loaded scenario set are available.
Persisted evidence restoration is separate background work and does not disable
a new run. Row commands resolve through the owning ItemsControl data context.
Persisted evidence remains subject to the model-content, scenario-definition,
evaluator-contract, and runtime-provenance checks described above.
