...
- **The nav and toolbar cursor flicker is fixed.** Confirmed rather than
  theorised this time: removing a tooltip from one button stopped that button
  flickering, so the tooltip popup was taking pointer-over and making the
  control count as exited. Tooltips are now not hit-testable, which keeps every
  tooltip and makes the loop impossible to form. This was the third attempt;
  the two before it fixed the cursor region, which was never the problem.
...
- **Moss is the application and tray icon**, replacing the Archivist's Seal
  monogram. The raster assets are generated by `scripts/generate-icons.ps1`
  from the mascot palette, so they can be regenerated rather than being opaque
  binaries. It is a programmatic approximation, legible at 16px; designed art
  can drop into the same filenames.
