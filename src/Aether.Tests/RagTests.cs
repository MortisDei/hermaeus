...
namespace Aether.Tests
{
    internal static class RagTests
    {
        ...
        private sealed class RecordingEmbeddingService : Aether.Rag.Embeddings.IEmbeddingService
        {
        ...
        /// </summary>
        private sealed class HashingBagOfWordsEmbeddingService : Aether.Rag.Embeddings.IEmbeddingService
        {
        ...
        public static async Task RagEvalRetrievalModeShouldRefuseCasePassesOnEmptyDatasetAndFailsWhenAnswerable()
        {
            ...
            var evalSet = new Aether.Rag.Models.RagEvalSet
            {
                Name = "gold",
                Cases = [new Aether.Rag.Models.RagEvalCase { Id = "refuse-1", Question = "anything at all", ShouldRefuse = true }]
            };
        ...
        public static async Task RagEvalCancellationStopsBetweenCasesAndSkipsExport()
        {
            ...
            {
                Name = "gold",
                Cases = Enumerable.Range(0, 5).Select(i => new Aether.Rag.Models.RagEvalCase { Id = $"case-{i}", Question = "apple" }).ToList()
            };
            await File.WriteAllTextAsync(evalPath, System.Text.Json.JsonSerializer.Serialize(evalSet));
        ...
        private sealed class CancelAfterNCallsEmbeddingService : Aether.Rag.Embeddings.IEmbeddingService
        {
        ...
        private sealed class ConstantEmbeddingService : Aether.Rag.Embeddings.IEmbeddingService
        {
            public int Dimensions => 4;
        ...
        private sealed class DistinctEmbeddingService : Aether.Rag.Embeddings.IEmbeddingService
        {
        ...
        private sealed class CollectingRuntimeLog : Aether.Core.Services.IRuntimeLogService
        {
            public List<Aether.Core.Models.RuntimeLogEntry> Entries { get; } = [];
            public event Action<Aether.Core.Models.RuntimeLogEntry>? LogAdded;
            public void Add(Aether.Core.Models.RuntimeLogEntry entry) { Entries.Add(entry); LogAdded?.Invoke(entry); }
            public IReadOnlyList<Aether.Core.Models.RuntimeLogEntry> GetEntries() => Entries;
