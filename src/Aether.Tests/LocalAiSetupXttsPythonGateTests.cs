...
public sealed class LocalAiSetupXttsPythonGateTests
{
    private const string PassingChecks = """
        AETHER_CHECK=Execute=PASS
        AETHER_CHECK=Version detection=PASS
        AETHER_CHECK=Import encodings=PASS
        AETHER_CHECK=Import venv=PASS
        AETHER_CHECK=Valid sys.prefix=PASS
        AETHER_CHECK=Create test venv=PASS
        """;
    ...
    private static string WriteStub(TempDir temp, string version)
    {
        ...
        var lines = new[] { "@echo off", $"echo AETHER_VERSION={version}" }
