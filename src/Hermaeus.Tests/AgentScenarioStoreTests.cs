...
public sealed class AgentScenarioStoreTests
{
    ...
    private static AgentScenarioStore NewStoreWithBuiltInRoot(Hermaeus.Core.Services.ISettingsService settings, string builtInRoot) =>
        new(settings, builtInRoot);

    /// <summary>Settings pointed at a temp data root so scenario-store tests never fall back to the real %LOCALAPPDATA%/Hermaeus/agent-scenarios.</summary>
