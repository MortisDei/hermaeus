...
internal static class VoicePronunciationTests
{
    ...
    public static Task UserLexiconSeedsDefaultsIncludingTheAppsOwnName()
    {
        ...
        var phonemes = KokoroPhonemizer.ToPhonemes("hermaeus", lexiconPath);
        Equal("hɚˈmiəs", phonemes, "Hermaeus's own name must be seeded into a fresh user lexicon file.");
    ...
    public static Task MorphologyResolvesPossessiveOfUserLexiconWord()
    {
        using var temp = new TempDir();
        var lexiconPath = temp.PathFor("voice/lexicon.txt");
        // Force the default seed (which includes "hermaeus") to be written first.
        KokoroPhonemizer.ToPhonemes("hermaeus", lexiconPath);

        Equal("hɚˈmiəsɪz", KokoroPhonemizer.ToPhonemes("hermaeus's", lexiconPath), "Possessive suffix morphology should reuse the stem's lexicon pronunciation plus a voiced 's'.");
        return Task.CompletedTask;
    }
    ...
    [
        ...
        "Hermaeus uses Ollama and Kokoro",
