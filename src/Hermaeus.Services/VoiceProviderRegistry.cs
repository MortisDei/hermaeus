    ...
    public IReadOnlyList<VoiceProviderInfo> GetAvailableProviders()
    {
        return new List<VoiceProviderInfo>
        {
            ...
            new VoiceProviderInfo(
                ...
                "Kokoro (native)",
                "Default voice provider. Fully in-process Kokoro: no Python subprocess, ONNX inference runs directly in Hermaeus. English voices only; downloads its model once via the Doctor install action, then runs fully offline.",
