...
app = FastAPI(title="Hermaeus XTTS v2 API")
tts_engine = None
settings = None
...
def speech(request: SpeechRequest):
    ...
    output_path = output_dir / f"hermaeus-{int(time.time())}-{uuid.uuid4().hex}.wav"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Hermaeus XTTS v2 API server")
